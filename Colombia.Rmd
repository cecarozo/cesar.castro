...
date: "Updated: May 30, 2018" # !!!! cambiar sombras (sumar 1 observación)
...
# GDP: annual supply
s.annual.table <- read_excel("/Users/cesarcastrorozo/Dropbox/SpinOff/colombia/datos2.xls", "cuadro2")
s.annual.table <- as.matrix(s.annual.table)
rownames(s.annual.table) <- c('Agriculture', 'Mining', 'Industry', 
                              'Household services', 'Construction', 'Trade',
                              'Comunication', 'Financial', 'Real estate',
                              'Professional', 'Public', 'Arts', 'Taxes', 'TOTAL')

# GDP: demand
d.annual.table <- read_excel("/Users/cesarcastrorozo/Dropbox/SpinOff/colombia/datos4.xls", "cuadro4")
d.annual.table <- as.matrix(d.annual.table)
rownames(d.annual.table) <- c('Private consumption', 'Public consumption',
                              'Gross capital formation', 'Exports', 'Imports',
                              'TOTAL')

# GDP: Figure
crecimiento <- read_excel("/Users/cesarcastrorozo/Dropbox/SpinOff/colombia/datos6.xls", "figura2")
crecactual <- ts(crecimiento[,1], start=c(2010,1), frequency=4)
crecprevio <- ts(crecimiento[,2], start=c(2010,1), frequency=4)

# Inflation: Figure
inflacion <- read_excel("/Users/cesarcastrorozo/Dropbox/SpinOff/colombia/datos10.xls", "figura3")
inflactual <- ts(inflacion[,1], start=c(2010,1), frequency=12)
inflprevio <- ts(inflacion[,2], start=c(2010,1), frequency=12)

# Inflation: errors
t.error.grupos <- read_excel("/Users/cesarcastrorozo/Dropbox/SpinOff/colombia/datos8.xls", "cuadro6")
t.error.grupos <- as.matrix(t.error.grupos)
rownames(t.error.grupos) <- c('TOTAL', 'Core', 'FOOD', 'Processed food', 'Non-processed food', 'Non-energy industrial goods', 'Services', 'Energy')

# Inflation: forecasts
p.table <- read_excel("/Users/cesarcastrorozo/Dropbox/SpinOff/colombia/datos9.xls",
                       "cuadro7")
p.table <- as.matrix(p.table[1:5,])

# Monetary policy
g <- read_excel("/Users/cesarcastrorozo/Dropbox/SpinOff/colombia/datos12.xls",
                       "figura8")
```
...
plot(crecactual, type='l', col = 'firebrick', lwd = 4, ylim=c(1, 9),
     yaxt='n', ylab='', xlab='', main='year-on-year rates')
rect(time(crecactual)[(66-32):(66-32)],
     10,
     time(crecactual)[40:40],
     1, density=50, col = "pink")
abline(h=c(2, 3, 4, 5, 6, 7, 8), col='gray90')
abline(v = c(2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019), col='gray90')  # year line
...
legend(2014, 9, c("current month", "previous month"),
       lty = 1, cex=.9, bg='white', col = c('firebrick','brown'),
       lwd=c(4, 2), bty='n', text.col=c('firebrick','brown'))
```

****
<!-- ### Innovations by sectors in the fourth quarter-2017 -->
...
colnames(s.annual.table) <- c('--2015--', '--2016--', '--2017--', '--2018--', '--2019--')

s.annual.table <- xtable(s.annual.table, caption='year-on-year rates. 2018 and 2019 are forecasts',
...
colnames(d.annual.table) <- c('--2015--', '--2016--', '--2017--', '--2018--', '--2019--')

d.annual.table <- xtable(d.annual.table, 
                         caption='year-on-year rates. 2018 and 2019 are forecasts',
...
plot(inflactual, type='l', col = "slateblue4", lwd = 3, ylim=c(1.5, 9.5),
     yaxt='n', ylab='', xlab='', bty='n', main='year-on-year rates')
rect(time(inflactual)[(218-118):(218-118)],
     1.5,
     time(inflactual)[120:120],
     9.5, density=50, col = "lightsteelblue1")
abline(h=c(2, 4), col='firebrick')
abline(h=c(3, 5, 6, 7, 8, 9), col='gray90')
abline(v = c(2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017,
             2018, 2019),
...
****
### Innovations by sectors in the inflation of April-2018
```{r Ierror, echo=FALSE, warning=FALSE, results='asis', message=FALSE}
colnames(t.error.grupos) <- c('--Weights--', '--Observed--', '--Forecast--',
                              '--Error--')
...
rownames(p.table) <- c('2015--', '2016--', '2017--', '2018--', '--2019--')
...
p.table <- xtable(p.table, caption='Year-on-year rates. 2018 and 2019 are forecasts', label='i.table', align=c('r|cc|cccc'), digits=1)
...
print(p.table,
      ...
      caption.placement = "top",
      sanitize.colnames.function = identity, type="html", include.rownames=T
)
```
...
g <- ts(g, start=c(2006,1), frequency=12)
...
rect(time(inflactual)[(100):(100)],
     1.5,
     time(inflactual)[120:120],
     9.5, density=50, col = "lightpink")
abline(h=c(3, 4, 5, 6, 7, 8), col='gray90')
abline(v = c(2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019), col='gray90')
axis(1, at = seq(2010, 2019, 1), tcl = 0.2)
